...
from ..utils.tools.scoring_tool import weighted_match_score
...
def _fallback_skill_gap(state: dict) -> dict:
    ...
    if not evidence_report:
        ...
        github_summary = (state.get("github_evidence_report") or {}).get("skill_evidence_summary", {})
        for skill in resume_skills:
            evidence_report[skill] = {
                "evidence": ["Mentioned in resume"],
                "confidence": 0.62,
                "status": "weak",
            }
        for skill, details in github_summary.items():
            evidence_report[skill] = {
                "evidence": [f"GitHub repos: {', '.join(details.get('repos', []))}"],
                "confidence": float(details.get("confidence", 0) or 0),
                "status": "strong" if float(details.get("confidence", 0) or 0) >= 0.75 else "weak",
            }
    score_data = weighted_match_score(state.get("job_research", {}), evidence_report)
